...
function PANEL:Init()
	self.ModelID = Player.PS2_Slots["Model"] and Player.PS2_Slots["Model"].id or 0
	...
	hook.Add( "PS2_ItemEquipped", "BGItemEquipped", function( ply, item )
		if Player != ply or !Pointshop2:IsPlayerModelEquipped() or Player.PS2_Slots["Model"].id != item.id then return end
